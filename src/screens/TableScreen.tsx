...
export default function TableScreen() {
    ...
    const toggleNutrientSelection = useCallback((nutrientName: string) => {
        setSelectedNutrients((current) => {
            ...
            return [...current, nutrientName].sort((a, b) =>
                a.localeCompare(b),
            );
        });
    }, []);
    ...
    const treeData = useMemo(() => {
        const tree = compactDescriptionTree(
            createDescriptionTree(filteredFoods),
        );
                    ...
                    >
                        {row.original.children.length === 0 &&
                            ...
                            visibleNutrients(
                                ...
                                <Chip
                                    key={index}
                                    color={NUTRIENT_COLOR_MAP[nutrient.name]}
                                >
                ...
                >
                    <Table.Body>
                        {table.getRowModel().rows.map((row) => (
                            ...
                            >
                                <Table.Row
                                    style={{
                                        backgroundColor: getDepthRowColor(
                                            row.depth,
                                        ),
                                    }}
                                >
                    ...
                    >
                        ...
                        {!isPending && !isFetchingNextPage && !hasNextPage ? (
                            <Text style={{ color: vars.colors.text.disabled }}>
                                End of list
                            </Text>
                        ) : null}

                        {error ? (
                            <Text style={{ color: vars.colors.text.alert }}>
